...
int main() {
    ...
    if (android::base::GetBoolProperty(kConfigDebugPathProperty.data(), false)) {
        ...
        LOG(WARNING) << "Power HAL AIDL Service for realme_sm6375 is using debug config from: " << config_path;
    }
    config_path.append(
            android::base::GetProperty(kConfigProperty.data(), kConfigDefaultFileName.data()));

    LOG(INFO) << "Power HAL AIDL Service for realme_sm6375 with Extension is starting with config: "
              << config_path;
    ...
    LOG(INFO) << "Power HAL AIDL Service for realme_sm6375 is started.";
    ...
    LOG(ERROR) << "Power HAL AIDL Service for realme_sm6375 with Extension just died.";
